...
// a cache line is 128 bytes if we could hit 32 bytes we'd have aligned
// accesses with no padding. We can do that by removing right idx since
// it's always left_idx + 1.
//
// Because an internal node always has children but no triangles, and a leaf
// node never has children and always triangles, we can store the
// left_idx and the prims_offset in the same field. This brings us to 32 bytes!
//
