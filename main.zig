...
fn hit_sphere(center: vec3f, radius: f32, ray: *const Ray) f32 {
...
fn ray_color(ray: *const Ray) vec3f {
    const t = hit_sphere(vec3f{0.0, 0.0, -1.0}, 0.5, ray);
    if (t > 0.0) {
        const hit_normal = vec3f_normalize(ray.at(t) - vec3f{0.0, 0.0, -1.0});
